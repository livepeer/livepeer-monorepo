...
_We used [`munpack`](https://linux.die.net/man/1/munpack) to unpack the multipart 
return.  To install `munpack`, use `brew install mpack` for macos, 
and `apt install mpack` for Linux. Alternatively, you can save the response directly
to a file. The content of the API response looks something like:_
```
