...
## Introduction to Livepeer API

Livepeer supports live streaming, video on demand, and transcoding across video
formats and protocols. The Livepeer API is provides enterprise customers a
secure, straightforward way to transcode video. Please note that the Livepeer
...
- `POST` - create a stream (Protected by `Bearer` API token auth)
- RESTFUL - getting streams by ID (Protected by `Bearer` API token auth)

`/broadcaster`

- `GET` a list of broadcasters

`/{broadcaster_url}/live/{stream_id}/{medianumber}.ts`

- ts segment ingest point after getting the broadcaster URL (using `POST`)

# Integration via HTTP Push API

## Requirements

Incoming live stream must be segmented into .ts video segments in h.264. We
...
Soon, API users will be able to send an RTMP stream to the Livepeer ingest point
and consume the transcoded HLS stream from the same broadcaster. Then the
transcoded HLS stream can be sent to an object store to serve as the CDN origin.
